...
-- time.
emptyTensor :: (Shape (Dim n), TensorScalar a, PrimMonad m)
...
fromList :: forall m n a . (PrimMonad m, Shape (Dim n), TensorScalar a)
...
fromList shp content = unsafePrimToPrim $ do
    ...
    withArray content $ \dataptr -> do
      tensor <- emptyTensor shp :: IO (IOTensor n a)
...
toList :: forall m n a . (PrimMonad m, Shape (Dim n), TensorScalar a)
...
-- | Tensor reshaping.
reshape :: (MonadError String m, Shape (Dim n), Shape (Dim k))
...
unsafeReshape :: (Shape (Dim n), Shape (Dim k))
              => Dim k -> MTensor st n a -> MTensor st k a
unsafeReshape newshp tensor = case reshape newshp tensor of
  Left err -> error err
  Right out -> out
